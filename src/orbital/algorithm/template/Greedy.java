 ...
 * the greedy algorithm in <a href="http://www-robotics.eecs.lehigh.edu/~bacon/setl-doc.html">SETL</a> goes like this:
 * <pre>
 ...
 *         <i>retract <var>x</var> from <span class="set">C</span> such that weight(<span class="set">S</span>,<var>x</var>) is maximal</i>;
 ...
 */
public
class Greedy implements AlgorithmicTemplate {
	...
	private final List solveByGreedy(GreedyProblem p) {
		List	   C = p.getInitialCandidates();
		List	   S = new LinkedList();
		Evaluation eval = new Evaluation(Selecting.best(), p.getWeightingFor(S));
		while (p.isPartialSolution(S) && !C.isEmpty()) {
			...
			// retract x from C such that weight(S,x) is maximal;
