 ...
 */
public
abstract class DynamicProgrammingOptimizingProblem implements DynamicProgrammingProblem {
	...
	public Object solve(int[] part, Object[] partialSolutions) {
		...
		Evaluation eval = new Evaluation(Selecting.best(), getWeightingFor(part));
		eval.addAll(options);
		Object optimum = eval.evaluate();

		// memorize weights as well to be dynamic
		DynamicProgramming.setSolutionPart(part, partialWeights, eval.getSelection().getWeight());
		return optimum;
	} 
}
