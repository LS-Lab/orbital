 ...
 * like:
 * <pre>
 * ga = <span class="keyword">new</span> <span class="Orbital">IncrementalGeneticAlgorithm</span>(<span class="Number">2</span>,<span class="Number">2</span>, maximumCrossover, maximumMutation);
 ...
 */
public
abstract class GeneticAlgorithm implements ProbabilisticAlgorithm, Serializable {
	 ...
	 * @serial
	 */
	private double		  maximumCrossover;

	/**
	 * Maximum probability rating of mutation level for reproducation.
	 * @serial
	 */
	 ...
	 *  Maximum probability rating of crossover level for reproducation.
	 */
	protected GeneticAlgorithm(int parentCount, int childrenCount, double maximumCrossover, double maximumMutation) {
		this();
		if (!MathUtilities.isProbability(maximumCrossover))
			throw new IllegalArgumentException("invalid crossover probability " + maximumCrossover);
		...
		this.maximumCrossover = maximumCrossover;
	...
	public Object clone() {
		...
		c.setMaximumCrossover(getMaximumCrossover());
	...
	public boolean equals(Object o) {
		if (o != null && getClass() == o.getClass()) {
			...
			if (childrenCount != b.childrenCount || parentCount != b.parentCount || maximumCrossover != b.maximumCrossover || maximumMutation != b.maximumMutation)
				return false;
	...
	/**
	 * Get the maximum probability rating of crossover level for reproducation.
	 */
	public double getMaximumCrossover() {
		return maximumCrossover;
	} 
	/**
	 * Set the maximum probability rating of crossover level for reproducation.
	 * @pre crossover&isin;[0,1] is a probability
	 */
	public void setMaximumCrossover(double crossover) {
		if (!MathUtilities.isProbability(crossover))
			throw new IllegalArgumentException("invalid probability " + crossover);
		maximumCrossover = crossover;
	} 
	/**
	 * Get the maximum probability rating of mutation level for reproducation.
	 */
	public double getMaximumMutation() {
		return maximumMutation;
	} 
	/**
	 * Set the maximum probability rating of mutation level for reproducation.
	 ...
	 * @see #selection
	 * @see Genome#crossover(Gene[],int,double)
	 * @see #maximumCrossover
	 * @see #maximumMutation
	 */
	 ...
	 * Parents are selected and will crossover and mutate to produce child genomes
	...
	/*protected*/ Genome[] reproduce() {
		...
		// crossover to children
		//@todo consider whether similarity * maximumCrossover would really be better?
		Genome children[] = (Genome[]) parents[0].crossover(parents, childrenCount, maximumCrossover);
	...
	public String toString() {
		return getClass().getName() + "[parentCount=" + parentCount + ",childrenCount=" + childrenCount +", maximumCrossover=" + maximumCrossover + ",maximumMutation=" + maximumMutation + "]";
	} 
}
